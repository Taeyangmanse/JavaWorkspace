...
public class ListRun 
{
	public static void main(String[] args) 
	{
		 ...
		 * - 효율적인 기능들이 정의되어 있는 틀
		 * 
		 * 데이터들이 새롭게 추가되거나, 삭제되거나, 수정이 되는 기능들을 이미 정의해 놓은 틀이다.
		 * 
		 * 배열과 컬렉션(List)의 차이점
		 * - 배열의 단점
		 * 1. 같은 자료형의 데이터만 저장 가능
		 * 2. 배열의 크기를 수정할 수 없다.
		 * 3. 배열 내부의 값들을 조작하기 불편하다.
		 * 
		 * - 컬렉션의 장점
		 * 1. 여러 자료형의 값을 저장할 수 있다.(제네릭 설정을 통해 한 가지 유형의 데이터만 저장하는 것도 가능하다.)
		 * 2. 중간에 값을 추가하거나, 삭제하는 등 배열 내부의 값들을 조작하기 위한 편리한 메서드가 내장되어 있다.
		 * 3. 크기에 대한 제약이 없다. (알아서 배열 깊은 복사를 해준다.)
		 * 
		 * 
		 * 컬렉션의 3가지 분류
		 * - List 계열
		 ...
		 * - Set 계열
		 *  - 담고자 하는 값만을 저장한다
		 *  - 값 저장 시 순서를 유지하지 않는다.(Index 없음)
		 *  - 중복값을 허용하지 않는다.
		 *  - HashSet, TreeSet
		 *  
		 * - Map 계열
		 *  - 담고자 하는 값(value)과 그 값을 저장하는 키(key)를 함께 저장
		 *  - 값 저장 시 순서를 유지하 지 않는다.(Index 없음)
		 *  - key값의 중복은 허용하지 않으나, value의 중복은 허용
		 *  - HashMap, Properties, ...
		 * 
		 */
		
		// List list = new ArrayList();	// 크기 10 배열 생성
		
		/*
		 *  제네릭(Generic) <E>
		 *  - 별도의 제네릭 설정 없이 컬렉션 객체 생성 시 제네릭은 Object로 설정된다.
		 *  - 제네릭은 설정하면 설정한 타입의 값들만을 컬렉션에 담을 수 있다.
		 *  
		 *  제네릭을 설정하는 이유
		 *  1. 명시한 타입의 객체만 저장 가능하도록 하여 "타입의 안정성"을 확보하기 위해서
		 *  2. 컬렉션에 저장된 객체를 꺼내서 사용할 때, 직접 형변환하는 작업을 없애기 위해서
		 *  
		 */
		
		ArrayList<Music> list = new ArrayList<>(3);	// 객체 생성 시에는 제네릭 생략 가능
		// -> 타입 추론 기능
		
		// [null, null, null]?
		System.out.println(list); 	// size 기준 내용 출력
		
		// 1. add(E e) : 리스트의 끝(초기화된 위치 기준)에 전달된 e를 추가시켜주는 메서드
		list.add(new Music("Tears", "소찬휘"));
		list.add(new Music("와", "이정현"));
		list.add(new Music("멍", "김정현"));
		...
		list.add(0, new Music("화개장터", "조영남"));
		
		// 중간에 값 추가 시 알아서 기존 값들을 한칸씩 뒤로 땡겨주는 깊은 복사가 내부적으로 발생한다.
		
		System.out.println(list);
		
		// 3. set(int index, E e)
		// - 리스트에 해당 인덱스의 값을 전달되는 e로 변경시켜주는 메서드
		list.set(2, new Music("순정", "코요테"));
		
		System.out.println(list);
		
		// 4. remove(int index)
		// - 리스트의 인덱스 위치의 값을 삭제시키는 메서드
		list.remove(3);
		...
		// 만약 제네릭 설정을 안했다면?
		Music m2 = (Music)list.get(0);	// 강제 형변환 필요
		...
		// - 리스트가 e를 포함하고 있다면 true, 아니라면 false
		System.out.println(iArr.contains(9));
		
		// 8. indexOf(E e) : int
		// - 매개 인자 요소가 몇 번째 인덱스에 있는지 그 값을 반환
		// - 없다면 -1 반환
...
//			iter.remove();	// list 내부의 값을 삭제하는 메서드
		...
		// - 리스트의 컬렉션에 있는 데이터를 모두 추가해주는 메서드
		list.addAll(list2);
...
//		list.clear();
		 ...
		 * - vo클래스에 직접 상속시켜서 사용
		 * - 해당 vo클래스의 "기본 정렬 조건"으로 사용한다.
		 * 
		 * 
		 * 2. comparator 인터페이스 상속
		 * - 기본 정렬 조건 외의 추가 정렬 조건을 만들고자 할 때 사용
		 * - vo클래스 이외의 별도 클래스에 comparator를 상속시켜서 구현한다.
		 * - 여러개의 정렬 조건을 만들 수 있다.
		 *  
		 * 
		 */
