package com.kh.chap03.stream.part03_optional;
...
public class OptionalMethod 
{
	 ...
	 * Optional
	 * - 자바 8부터 추가된 클래스
	 * - 스트림에서 NPE, 기타 에러 상황들을 방지하기 위한 용도로 사용하는 클래스
	 * - Optional은 데이터를 직접 다루는 대신, optional 객체를 통해 간접적으로 다룬다.
	 * - Optional을 객체로 사용할 때 Optional 내부에 데이터가 있는지 없는지 항상 확인하는 절차를 강제함으로써 에러를 방지한다
	 * - Optional 객체를 변수에 저장하여 관리하는 경우, 변수명에 maybe, opt와 같은 Optional 타입임을 암묵적으로 제시한다.
	 * 
	 */
	...
	public static void main(String[] args) 
	{
		 ...
		 * 2. Optional.of(obj) : 결코 null이 아닌 객체를 감은 Optional객체를 생성
		 * 3. Optional.ofNullable(obj) : null 일 수도 있는 Optional 객체 생성
		 * 
		 * Optional 객체 내부 메서드들 
		 ...
		 * - orElseGet(supplier) : get() 으로 꺼낼 값이 존재하지 않을 경우 람다식을 통해 구현한 값을 반환
		 * - orElseThrow(supplier) : get()으로 꺼낼 값이 없을 때, 람다식을 통해 구현한 에러객체를 호출 
		 * ...
		 * 
		 */
...
//				.orElse(0); 기본값 방식
	...
	public void test2()
	{
		...
		member.setEmail("asdasd@asd.asd");
	...
	{
		// Optional 없이 최대한 안전(NPE)하게 email추출하기.
//		if (order != null)
//		{
